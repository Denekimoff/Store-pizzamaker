...
export const Sort = () => {
//Состояние видимости окна сортировки:
  const [isVisibleSort, setIsVisibleSort] = React.useState(false)
//Состояние активной вкладки сортировки:
                    ...
                    <div className="sort__popup">
                      <ul>
                        {sortList && sortList.map((item, index) => (
                          <li key={index} onClick={() => handleClickSelected(index)} className={selected === index ? 'active' : ''}>
                            {item}
                          </li>
                        ))}
                    </ul>
